...
Dispatch each task to a **fresh executor** — a new `swe-implementation` subagent (or a layer specialist — `swe-frontend`/`swe-backend`/etc. — when the task is layer-specific). Because the executor starts with no session history, every prompt must carry the **complete delegation contract**: objective, scope, relevant files, constraints, prior findings, acceptance criteria, known failures, required output, and the verification step — alongside the task's ID and Build details. Do not pass unrelated session history. Fresh context per task is deliberate: it prevents the silent drift that long sessions accumulate and forces the task to stand on its own. You remain the owner: you order the tasks, dispatch, review, and report.
...
Tasks in the same phase with **no ordering dependency and no shared files** may be dispatched in parallel. Verify independence from the complete plan, repository file set, generated outputs, and shared resources **before** dispatching — task text alone can omit shared package manifests, lockfiles, generated files, external state, or outputs another task consumes: no two tasks in the batch may edit the same file or consume each other's output. If you cannot confirm that, run them sequentially — parallel work you can't prove independent is where conflicts are born.
