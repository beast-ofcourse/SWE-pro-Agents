...
**26 OpenCode agent profiles (22 subagents + 4 primary) + 22 skills — a full engineering team in your terminal.**
...
- **22 on-demand skills** — token compression, skill authoring, tutoring, README/SVG/flowchart generation, an anti-AI-slop editor, OpenCode skill creation, and a full design-taste suite (brand kits, image-to-code, mobile/web image direction, UI styles, redesigns).
...
The postinstall hook copies agents to `~/.config/opencode/agents/swe-pro-agents/`, all 22 skills to `~/.config/opencode/skills/`, and this pack's `AGENTS.md` to the agents dir. That last file matters: every agent is intentionally short because it assumes the Engineering Operating System is loaded. If you have no global `~/.config/opencode/AGENTS.md`, copy the installed one into place:
...
The pack ships **22 skills**, each a self-contained `SKILL.md` loaded on demand via OpenCode's `skill` tool:
